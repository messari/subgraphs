...
import {
  ...
  errorNotification,
  fetchMessages,
  getAllThreadsToClear,
  getDiscordMessages,
  postAlert,
  sendMessageToAggThread,
} from "./messageDiscord.js";
import "dotenv/config";
import { protocolDerivedFields, protocolLevel } from "./protocolLevel.js";
import { errorsObj, protocolErrors } from "./errorSchemas.js";
import {
  pullMessagesByThread,
  resolveQueriesToAttempt,
  resolveThreadCreation,
} from "./resolutions.js";
import {
  generateDecenEndpoints,
  generateEndpoints,
  generateProtocolToBaseMap,
  indexStatusFlow,
  queryDecentralizedIndex,
} from "./indexingStatus.js";
...
try {
  ...
  errorNotification(
    "ERROR LOCATION 21 " + err.message + " MAIN LOGIC script.js"
  );
}
...
async function executionFlow() {
  ...
  Object.entries(subgraphEndpoints).forEach(
    ([protocolType, protocolsOnType]) => {
      Object.entries(protocolsOnType).forEach(([protocolName, protocolObj]) => {
        Object.entries(protocolObj).forEach(([network, deploymentString]) => {
          const deploymentData = Object.values(
            data[protocolName]?.deployments
          )?.find((x) => x.network === network);
          const status = deploymentData?.status || "dev";
          const versions = deploymentData?.versions || {
            schema: "N/A",
            subgraph: "N/A",
            methodology: "N/A",
          };
          const nameStr = deploymentString.split("name/")[1];
          ...
          deployments[deploymentsKey].protocolErrors = JSON.parse(
            JSON.stringify(protocolErrors)
          );
          if (
            protocolType &&
            deploymentsKey &&
            Object.keys(errorsObj).includes(protocolType)
          ) {
            deployments[deploymentsKey].poolErrors = JSON.parse(
              JSON.stringify(errorsObj[protocolType])
            );
          }
          ...
          if (
            decenKeyToEndpoint[hostedEndpointToDecenNetwork[deploymentString]]
          ) {
            const decenObj =
              decenKeyToEndpoint[
            ...
            decentralizedDeployments[
              deploymentsKey + "(DECEN)"
            ].protocolErrors = JSON.parse(JSON.stringify(protocolErrors));
            if (
              protocolType &&
              deploymentsKey + "(DECEN)" &&
              Object.keys(errorsObj).includes(protocolType)
            ) {
              decentralizedDeployments[deploymentsKey + "(DECEN)"].poolErrors =
                JSON.parse(JSON.stringify(errorsObj[protocolType]));
            }
          }
        });
      });
  ...
  protocolNames.forEach((protocolName) => {
    const existingProtocolThread = currentDiscordMessages.find((msg) =>
      msg.content.includes(protocolName + " (Base")
    );
  ...
  const issuesMapping = await pullMessagesByThread(
    Object.keys(channelToProtocolIssuesMapping),
    channelToProtocolIssuesMapping,
    channelToIndexIssuesMapping
  );
  const issuesGithub = await getGithubIssues();
  const currentThreadMessages = await fetchMessages(
    "",
  ...
  const messagesToPost = protocolNames.map((protocolName) => {
    ...
    const deploymentSet = Object.values(deployments).filter(
      (depo) => depo.protocolName === protocolName
    );
    ...
    return constructEmbedMsg(
      protocolName,
      deploymentSet,
      protocolIssuesOnThread,
      indexDeploymentIssues,
      channelId,
      issuesGithub,
  ...
  const aggThread = currentDiscordMessages.find((x) =>
    x.content.includes("Production Ready Subgraph Indexing Failure")
  );
