...
async function executionFlow() {
  ...
  const loopDeploymentJSON = await generateEndpoints(
    data,
  ...
  const channelMapping = await resolveThreadCreation(
  ...
  let messagesToPost = protocolNames.map((protocolName) => {
    ...
    const embeddedMessages = constructEmbedMsg(
      protocolName,
      deploymentSet,
  ...
  if (messagesToPost.length > 0) {
    ...
    messagesToPost = messagesToPost.filter((msg, idx) => {
      if (!msg?.channel && !!msg) {
        messagesToPost[idx].channel =
          protocolNameToChannelMapping[msg?.protocolName];
      }
      return !!msg;
    });
