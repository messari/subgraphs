...
import { clearThread, constructEmbedMsg, errorNotification, getAllThreadsToClear, getDiscordMessages, postAlert, sendMessageToAggThread } from "./messageDiscord.js";
...
async function executionFlow() {
  ...
  let messagesToPost = protocolNames.map(protocolName => {
    ...
    const embeddedMessages = constructEmbedMsg(protocolName, deploymentSet, protocolIssuesOnThread, indexDeploymentIssues, issuesGithub);
    if (embeddedMessages) {
  ...
  if (messagesToPost.length > 0) {
    ...
    console.log(aggThreadId, currentDiscordMessages.length)
    if (aggThreadId) {
