...
export async function pullMessagesByThread(
  channelIdList,
  channelToProtocolIssuesMapping,
  channelToIndexIssuesMapping
) {
  const channelToProtocolIssuesMappingCopy = JSON.parse(
    JSON.stringify(channelToProtocolIssuesMapping)
  );
      ...
      if (res?.value?.length > 0) {
        res.value.forEach((protocolMessageObject) => {
          const networkList = Object.keys(
            channelToProtocolIssuesMappingCopy[
              protocolMessageObject.channel_id
            ] || []
          );
          ...
          networkList.forEach((chain) => {
            ...
            const embedObjectOnChain = protocolMessageObject.embeds.find(
              (x) =>
                x.title.includes(chainStr) &&
                x.title.toUpperCase().includes("PROTOCOL LEVEL ERRORS") &&
                ((x.title.toUpperCase().includes("PENDING") && isPending) ||
                  (!x.title.toUpperCase().includes("PENDING") && !isPending))
            );
            if (!embedObjectOnChain) {
              return;
            }
            let fieldCells = embedObjectOnChain.fields
              .filter((cell) => cell.name === "Field")
              .map((x) => x.value);
            if (fieldCells) {
              ...
              fieldCells = [
                ...existingFields,
                ...fieldCells.filter((x) => !existingFields.includes(x)),
              ];
              channelToProtocolIssuesMappingCopy[
                protocolMessageObject.channel_id
              ][chainStr] = fieldCells;
          ...
          const indexingErrorObject = protocolMessageObject.embeds.find((x) =>
            x.title.toUpperCase().includes("INDEXING ERRORS")
          );
          if (!indexingErrorObject) {
            return;
          }
          const chainLine = indexingErrorObject.fields.find(
            (x) => x.name === "Chain"
          )?.value;
          if (!chainLine) {
            return;
          }
          const indexedLine = indexingErrorObject.fields.find(
            (x) => x.name === "Failed At Block"
          )?.value;
          if (!indexedLine) {
            return;
          }
          const linesByChainArr = chainLine
            .split("\n")
            .join("-----")
            .split("-----");
          const linesByIndexedArr = indexedLine
            .split("\n")
            .join("-----")
            .split("-----");
          linesByChainArr.forEach((line, idx) => {
            channelToIndexIssuesMapping[protocolMessageObject.channel_id].push({
              chain: line,
              indexed: linesByIndexedArr[idx],
              timestamp: protocolMessageObject.timestamp,
            });
          });
        });
      }
  ...
  return {
    channelToProtocolIssuesMapping: channelToProtocolIssuesMappingCopy,
    channelToIndexIssuesMapping,
  };
}

let reqCount = 0;
export async function resolveThreadCreation(
  protocols,
  threadsCreated,
  ...
  try {
    const promiseSettle = await Promise.allSettled(
      useQueries.map((object) => {
...
export async function resolveQueriesToAttempt(queriesToAttempt) {
  ...
  try {
    await Promise.allSettled(
      useQueries.map((object) =>
        sendDiscordMessage(object.message, object.protocolName, object.channel)
      )
    );
  } catch (err) {
