...
export const protocolLevel = async (deployments, invalidDeployments) => {
  ...
  protocolTypeLevelData.forEach((protocol) => {
    ...
    if (
      !(
        data.totalValueLockedUSD >= 0 && data.totalValueLockedUSD < 100000000000
      )
    ) {
      issuesArrays.totalValueLockedUSD.push(
        "$" + formatIntToFixed2(parseFloat(data.totalValueLockedUSD))
      );
    }
    ...
    if (
      ...
      issuesArrays.cumulativeSupplySideRevenueUSD.push(
    ...
    if (
      ...
      issuesArrays.cumulativeProtocolSideRevenueUSD.push(
        "$" +
    ...
    if (
      Math.abs(
        parseFloat(data.cumulativeProtocolSideRevenueUSD) +
          ...
          parseFloat(data.cumulativeTotalRevenueUSD)
      ) > 100
    ) {
      const value =
        "$" +
    ...
    if (
      ...
      issuesArrays.cumulativeVolumeUSD.push(
        "$" + formatIntToFixed2(parseFloat(data.cumulativeVolumeUSD))
      );
    }
    ...
    if (
      ...
      !(
        ...
        parseFloat(data.totalPoolCount) < 10000
      )
    ) {
    ...
    if (
      ...
      issuesArrays.cumulativeUniqueDepositors.push(
        data.cumulativeUniqueDepositors
      );
    }
    ...
    if (
      ...
      issuesArrays.cumulativeUniqueBorrowers.push(
        data.cumulativeUniqueBorrowers
      );
    }
    ...
    if (
      ...
      issuesArrays.cumulativeUniqueLiquidators.push(
        data.cumulativeUniqueLiquidators
      );
    }
    ...
    if (
      ...
      issuesArrays.cumulativeUniqueLiquidatees.push(
        data.cumulativeUniqueLiquidatees
      );
    }
    ...
    if (
      ...
      issuesArrays.totalDepositBalanceUSD.push(
    ...
    if (
      ...
      issuesArrays.cumulativeDepositUSD.push(
        "$" + formatIntToFixed2(parseFloat(data.cumulativeDepositUSD))
      );
    }
    ...
    if (
      ...
      issuesArrays.totalBorrowBalanceUSD.push(
        "$" + formatIntToFixed2(parseFloat(data.totalBorrowBalanceUSD))
      );
    }
    ...
    if (
      ...
      issuesArrays.cumulativeLiquidateUSD.push(
        "$" + formatIntToFixed2(parseFloat(data.cumulativeLiquidateUSD))
      );
    }
...
export const protocolDerivedFields = async (
  deployments,
  invalidDeployments
) => {
  ...
  await Promise.allSettled(derivedFieldQueriesToMake)
    .then((response) =>
      response.map((x) => {
        ...
        const depoKey = Object.keys(deployments).find(
          (depo) => deployments[depo].url === x.value.config.url
        );
        let alert = ``;
        ...
        if (returnedData) {
          ...
          if (returnedData[key]?.length === 0 || !returnedData[key]) {
            ...
            deploymentsToReturn[depoKey].protocolErrors.protocolEntity.push(
              ProtocolTypeEntityName[deploymentsToReturn[depoKey].protocolType]
            );
          } else {
            const emptyFields = Object.keys(returnedData[key][0]).filter(
              (field) =>
                !returnedData[key][0][field] ||
                returnedData[key][0][field]?.length === 0
            );
            if (emptyFields.length > 0) {
              deploymentsToReturn[depoKey].protocolErrors.relatedField.push(
                emptyFields.join(", ")
              );
            }
          }
        ...
        if (returnedError) {
          const alertArr = returnedError
            .filter((errObj) => errObj.message !== "indexing_error")
