...
export const protocolLevel = async (deployments) => {
  ...
  protocolTypeLevelData.forEach((protocolData) => {
    ...
    if (
      ...
      issuesArrays.cumulativeRevenueFactors.push(
        "$" +
    ...
    if (
      ...
      !issuesArrays.cumulativeVolumeUSD.includes(
    ...
    const userTypesStr = `${data.cumulativeUniqueDepositors || 0}+${
      data.cumulativeUniqueBorrowers || 0
    }+${data.cumulativeUniqueLiquidators || 0}+${
      data.cumulativeUniqueLiquidatees || 0
    }=${data.cumulativeUniqueUsers || 0}`;
    if (
    ...
    if (
      ...
      !issuesArrays.cumulativePositionCount.includes(
        data.cumulativePositionCount
      )
    ) {
    ...
    const totalBorrowBalanceUSDExcluded = [
      "rari-fuse",
      "truefi",
      "maple",
      "goldfinch",
    ...
    if (
      ...
      !issuesArrays.totalBorrowBalanceUSD.includes(
    ...
    if (
      ...
      !issuesArrays.cumulativeLiquidateUSD.includes(
      ...
      response.map((x) => {
        ...
        if (returnedData && depoKey) {
          ...
          if (returnedData[key]?.length === 0 || !returnedData[key]) {
            ...
            const emptyFields = Object.keys(firstVal).filter(
              (field) => !firstVal[field] || firstVal[field]?.length === 0
            );
            if (
              emptyFields.length > 0 &&
              !depoKey.toUpperCase().includes("THE-GRAPH")
            ) {
        ...
        if (returnedError?.length > 0) {
          const alertArr = returnedError
            ?.filter(
              (errObj) =>
                errObj.message !== "indexing_error" &&
                !errObj.message.includes("Store error: database unavailable")
            )
          ...
          if (alertArr.length > 0) {
            ...
            if (depoKey) {
              deploymentsToReturn[depoKey].protocolErrors.queryError.push(
                alert
              );
            }
          }
