...
export async function errorNotification(
  ...
  try {
    const baseURL =
      "https://discordapp.com/api/channels/1019063880040861806/messages";
    const headers = {
    ...
    const postJSON = JSON.stringify({
      content:
        `**Subgraph Bot Monitor from ${process.env.CHANNEL_ID} on Channel ${channelId}- Errors detected**\n` +
        error,
    });
...
export async function clearMessages(channelId = process.env.CHANNEL_ID) {
  ...
  try {
    ...
    const postJSON = JSON.stringify({
      messages: msgs.map((x) => x.id).slice(0, 100),
    });
...
export async function getAllThreadsToClear(
  deleteMsgsFromBeforeTS,
  channelId = process.env.CHANNEL_ID
) {
  const msgs = await getDiscordMessages([], channelId);
...
export async function clearAllThreads(msgs, deleteMsgsFromBeforeTS) {
  ...
  try {
    await Promise.allSettled(
      useMsgs.map((msg) => clearThread(deleteMsgsFromBeforeTS, msg.id))
    );
  } catch (err) {
...
export async function clearThread(
  ...
  try {
    ...
    messages = msgs
      .filter((x) => moment(new Date(x.timestamp)).isSameOrBefore(timeAgo))
      .map((x) => x.id)
      .slice(0, 100);
    messages.pop();
...
export function constructEmbedMsg(
  protocol,
    ...
    if (Object.keys(indexErrorEmbedDepos)?.length > 0) {
      ...
      Object.keys(indexErrorEmbedDepos)?.forEach((networkString) => {
        if (networkString.includes(" (PENDING")) {
          ...
          }-PENDING](https://okgraph.xyz/?q=${
            indexErrorPendingHash[networkString]
          })\n`;
        } else {
...
export async function sendMessageToAggThread(
  ...
  aggThreadMsgObjects.forEach((aggThread) => {
    ...
    const msg = currentThreadMessages.find((x) => {
      return (
        !!x.embeds.find((embed) =>
          embed.title.toUpperCase().includes(aggThread.protocol)
        ) && moment(new Date(x.timestamp)).isSameOrAfter(messagesAfterTS)
      );
    });
    ...
    if (aggThread.embeds.length > 1) {
      if (!!msg) {
        const existingEmbed = msg.embeds.find((x) =>
          x.title.toUpperCase().includes("INDEXING ERRORS")
        );
        const aggThreadNetworkStringsArr = aggThread.embeds[0].value
          .split("\n")
          .join("-----")
          .split("-----");
        const aggThreadBlockValueArr = aggThread.embeds[1].value
          .split("\n")
          .join("-----")
          .split("-----");
        const existingMessageNetworkStringsArr =
          existingEmbed.embeds[0].value.fields
            .split("\n")
            .join("-----")
            .split("-----");
        const existingMessageBlockValueArr =
          existingEmbed.embeds[1].value.fields
            .split("\n")
            .join("-----")
            .split("-----");
    ...
    if (aggThreadMsgObjectsToSend.length > 0) {
      ...
      const query = axios
        ...
        .catch((err) => console.log(aggThread.protocol));
