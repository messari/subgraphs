...
import {
  ...
  sleep,
  colorsArray,
  parseIndexingThreadStrings,
  indexingErrorEmbedSchema,
} from "./util.js";
...
export async function postAlert(message) {
  try {
    ...
    const postJSON = JSON.stringify({
      content:
        `**Subgraph Bot Monitor from ${process.env.CHANNEL_ID} on Channel ${process.env.CHANNEL_ID}- Errors detected**\n` +
        message,
    });
    await axios.post(baseURL, postJSON, { headers: { ...headers } });
  } catch (err) {
    errorNotification(
      "ERROR POSTING DISCORD - " + err.message + " - " + message
    );
  }
}
...
export async function getDiscordMessages(
  ...
  try {
    const tempMessages = await fetchMessages(
...
export async function clearChannel(channelId) {
  ...
  try {
    ...
    const deletions = msgs.map((msg) => {
      return axios
        .delete(baseURL + msg.id, { headers: { ...headers } })
...
export async function clearMessages(channelId) {
  ...
  try {
    ...
    if (messages.length > 1) {
      await axios.post(baseURL + "/messages/bulk-delete", postJSON, {
        headers: { ...headers },
      });
...
export async function clearAllThreads(msgs, deleteMsgsFromBeforeTS) {
  ...
  try {
    await Promise.allSettled(
      useMsgs.map((msg) => clearThread(deleteMsgsFromBeforeTS, msg.channel_id))
    );
  } catch (err) {
...
export async function clearThread(deleteMsgsFromBeforeTS, channelId) {
  ...
  try {
    ...
    messages = msgs.filter((x) =>
      moment(new Date(x.timestamp)).isSameOrBefore(timeAgo)
    );
...
export async function sendDiscordMessage(
  ...
  const postJSON = JSON.stringify({
    content: `**Subgraph Bot Monitor - Errors detected on ${protocolName}**\n`,
    embeds: messageObjects,
  });
  try {
    const data = await axios.post(baseURL, postJSON, {
      headers: { ...headers },
    });
    return data;
  } catch (err) {
...
export async function startProtocolThread(
  subject,
  base,
  channelId = process.env.CHANNEL_ID
) {
  ...
  let msgId = "";
  try {
    const res = await axios.post(baseURL, postJSON, {
      headers: { ...headers },
    });
    msgId = res.data.id;
  } catch (err) {
...
export async function constructEmbedMsg(
  protocol,
  deploymentsOnProtocol,
  issuesOnThread,
  ...
  const embedObjects = [];
  const indexingErrorEmbed = JSON.parse(
    JSON.stringify(indexingErrorEmbedSchema)
  );
  ...
  const aggThreadIndexErrorEmbeds = JSON.parse(
    JSON.stringify([...indexingErrorEmbed.fields])
  );
  ...
  try {
    deploymentsOnProtocol.forEach((depo) => {
      ...
      const protocolErrorEmbed = {
        ...
        description:
          "After mapping through all of the subgraph deployments for this protocol, The errors listed in this section were detected within protocol level data.",
        fields: [],
      ...
      if (!!depo.indexingError && !depo?.protocolType?.includes("governance")) {
        ...
        if (!!depo.pending) {
          issueHasBeenAlerted =
            indexDeploymentIssues.find(
              (x) =>
                x.chain.includes(networkString.split(" ")[0] + "-PENDING") &&
                x.indexed.includes(depo?.indexingError?.toString()) &&
                moment(new Date(x.timestamp)).isSameOrAfter(messagesAfterTS)
            ) || false;
        } else {
          issueHasBeenAlerted =
            indexDeploymentIssues.find(
              (x) =>
                x.chain.includes(networkString) &&
                x.indexed.includes(depo?.indexingError?.toString()) &&
                moment(new Date(x.timestamp)).isSameOrAfter(messagesAfterTS)
            ) || false;
        }
        if (!issueHasBeenAlerted) {
          ...
          if (depo.isDecen) {
            indexErrorEmbedDepos[networkString + " (DECEN)"] = {
              failureBlock: depo?.indexingError,
              message: depo?.indexingErrorMessage,
            };
      ...
      if (protocol.split("-").pop() !== "swap") {
        ...
        Object.entries(depo.protocolErrors).forEach(
          ([errorType, errorArray], idx) => {
            ...
            if (errorArray.length > 0) {
              ...
              errorArray.forEach((error) => {
                protocolRows.push(
                  { name: "Field", value: errorType, inline: true },
                  { name: "Value", value: error, inline: true },
                  {
                    name: "Description",
                    value: protocolErrorMessages[errorType]
                      .split("'Protocol'")
                      .join(`${ProtocolTypeEntityName[depo.protocolType]}`)
                      .split("Value")
                      .join(error),
                    inline: true,
                  },
      ...
      if (protocolErrorEmbed.fields.length >= 3) {
        ...
        if (depo?.status === "prod") {
          ...
          zapierProdThreadProtocols[networkString] = {
            Field: [],
            Value: [],
            Description: [],
          };
          protocolErrorEmbed.fields.forEach((row) => {
            if (row.name === "Field") {
              ...
              zapierProdThreadProtocols[networkString]["Description"].push(
                row.value
              );
            }
          });
    ...
    errorNotification(
      "ERROR LOCATION constructEmbedMsg() initializing embed object " +
        err.message
    );
  }
  ...
  try {
    if (Object.keys(indexErrorEmbedDepos)?.length > 0) {
      ...
      Object.keys(indexErrorEmbedDepos)?.forEach((networkString) => {
        ...
        if (prodStatusDepoMapping[baseNetworkString] === true) {
          if (
            !(
              indexErrorObj.message.indexOf(
                "Failed to call 'asc_type_id' with 'ArrayBuffer'"
              ) >= 0
            )
          ) {
            aggThreadIndexErrorEmbeds[0].value += labelValueLine;
            aggThreadIndexErrorEmbeds[1].value += failureBlockLine;
            zapierProdThreadIndexing.push({
              zappierMessage: `${networkString}: ${indexErrorObj.failureBlock} - ${link}`,
              [threadKey]: `${networkString}: Block #${indexErrorObj.failureBlock}\nLink: ${link}\nError: ${indexErrorObj.message}\n\n`,
            });
          }
        }
      });
    ...
    errorNotification(
      "ERROR LOCATION constructEmbedMsg() generating specialized embed messages " +
        err.message
    );
  }
  ...
  try {
    if (embedObjects.length > 0) {
      ...
      aggThreadMsgObjects.push({
        embeds: indexingErrorEmbedsToAggThread,
        protocol: protocol,
        protocolErrorEmbeds: aggThreadProtocolErrorEmbeds,
      });
      await sendDiscordMessage(embedObjects, protocol, channelId);
    }
  } catch (err) {
    errorNotification(
      "ERROR LOCATION constructEmbedMsg() sending discord message " +
        err.message
    );
  }
  ...
  try {
    let object = {};
    if (
      Object.keys(zapierProdThreadProtocols).length > 0 ||
      zapierProdThreadIndexing.length > 0
    ) {
      if (zapierProdThreadIndexing.length > 0) {
        object = await constructZapierAndGithubIssue(
          {
            indexing: zapierProdThreadIndexing,
      ...
      if (Object.keys(zapierProdThreadProtocols).length > 0) {
        object = await constructZapierAndGithubIssue(
          {
            protocol: zapierProdThreadProtocols,
            protocolName: protocol,
            chains,
          },
          issuesGithub,
    ...
    errorNotification(
      "ERROR LOCATION constructEmbedMsg() sending zapier/gh embed " +
        err.message
    );
  }
  return;
...
export async function sendMessageToAggThread(
  aggThreadId = process.env.PROD_CHANNEL
) {
  ...
  try {
    ...
    aggThreadMsgObjects.forEach((aggThread) => {
      ...
      const msg = currentThreadMessages.find((x) => {
        const isAfterTs = moment(new Date(x.timestamp)).isSameOrAfter(
          messagesAfterTS
        );
        return (
          !!x.embeds.find((embed) =>
            embed.title.toUpperCase().includes(aggThread.protocol.toUpperCase())
          ) && isAfterTs
        );
      });
      ...
      if (aggThread.embeds.length > 1) {
        if (!!msg) {
          existingEmbed = msg.embeds.find((x) =>
            x.title.toUpperCase().includes("INDEXING ERRORS")
          );
          if (existingEmbed) {
            const aggThreadNetworkStringsArr = parseIndexingThreadStrings(
              aggThread.embeds[0]
            );
            const aggThreadBlockValueArr = parseIndexingThreadStrings(
              aggThread.embeds[1]
            );
            const existingMessageNetworkStringsArr = parseIndexingThreadStrings(
              existingEmbed.fields[0]
            );
            const existingMessageBlockValueArr = parseIndexingThreadStrings(
              existingEmbed.fields[1]
            );
            aggThreadNetworkStringsArr.forEach((networkLine, networkIdx) => {
              const existingMessageIndex =
                existingMessageNetworkStringsArr.indexOf(networkLine);
              if (
                !(
                  existingMessageIndex >= 0 &&
                  aggThreadBlockValueArr[networkIdx] ===
                    existingMessageBlockValueArr[existingMessageIndex]
                )
              ) {
                indexingErrorEmbed.fields[0].value += networkLine;
                indexingErrorEmbed.fields[1].value +=
                  aggThreadBlockValueArr[networkIdx];
                embedToAdd = true;
              }
            });
          ...
          aggThread?.embeds[0]?.value?.length > 0 &&
          aggThread?.embeds[1]?.value?.length > 0
        ) {
      ...
      if (aggThread.protocolErrorEmbeds) {
        aggThreadMsgObjectsToSend = [
          ...aggThreadMsgObjectsToSend,
          ...aggThread.protocolErrorEmbeds,
        ];
      }
      if (aggThreadMsgObjectsToSend.length > 0) {
        const postJSON = JSON.stringify({
          content: `**Subgraph Bot Monitor - Errors detected on ${aggThread.protocol} subgraphs (prod)**\n`,
          embeds: aggThreadMsgObjectsToSend,
        });
        const query = axios
          .post(baseURL, postJSON, { headers: { ...headers } })
          .catch((err) => console.log(err.message));
...
export async function constructZapierAndGithubIssue(
  msgObj,
  postedIssues,
  issuesProdDiscord
) {
  ...
  if (Object.keys(msgObj).includes("indexing")) {
    ...
    const threadIndexingAlerts = currentThreadMessagesContent.filter(
      (x) =>
        x.toUpperCase().includes("INDEXING") &&
        x.toUpperCase().includes(msgObj.protocolName.toUpperCase()) &&
        ((!msgObj.protocolName.toUpperCase().includes("PENDING") &&
          !x.toUpperCase().includes("PENDING")) ||
          (msgObj.protocolName.toUpperCase().includes("PENDING") &&
            x.toUpperCase().includes("PENDING")))
    );
    ...
    threadIndexingAlerts.forEach((indexingThread) => {
      msgObj.indexing.forEach((indexingAlert, idx) => {
        if (
          indexingThread
            .toUpperCase()
            .includes(indexingAlert.zappierMessage?.toUpperCase())
        ) {
          invalidIndexingAlertIndexes.push(idx);
        }
      });
    });
    const validAlerts = msgObj.indexing.filter(
      (x, idx) => !invalidIndexingAlertIndexes.includes(idx)
    );
    if (validAlerts.length > 0) {
      messageConstruction += `Indexing errors on ${msgObj.protocolName}\n\n`;
      messageConstruction += validAlerts
        .map((x) => x.zappierMessage)
        .join("\n");
      if (validAlerts.filter((x) => x.ghMessage).length > 0) {
        const titleObj = {
          protocol: msgObj.protocolName,
          type: "Indexing Errors",
          chains: msgObj.chains.filter(
            (x) => !x.toUpperCase().includes("DECEN")
          ),
        };
        ghIssuePromiseArray.push(
          postGithubIssue(
            titleObj,
            validAlerts
              .filter((x) => x.ghMessage)
              .map((x) => x.ghMessage)
              .join("\n"),
            postedIssues,
            false
      ...
      if (validAlerts.filter((x) => x.ghMessageDecen).length > 0) {
        const titleObj = {
          protocol: msgObj.protocolName,
          type: "Indexing Errors",
          chains: msgObj.chains.filter((x) =>
            x.toUpperCase().includes("DECEN")
          ),
        };
        ghIssuePromiseArray.push(
          postGithubIssue(
            titleObj,
            validAlerts
              .filter((x) => x.ghMessageDecen)
              .map((x) => x.ghMessageDecen)
              .join("\n"),
            postedIssues,
            true
          )
    ...
    Object.keys(msgObj.protocol).forEach((deployment) => {
      ...
      const threadProtocolAlerts = currentThreadMessagesContent.filter((x) => {
        return (
          x.toUpperCase().includes("PROTOCOL") &&
          x.toUpperCase().includes(deployment.toUpperCase()) &&
          ((!deployment.toUpperCase().includes("PENDING") &&
            !x.toUpperCase().includes("PENDING")) ||
            (deployment.toUpperCase().includes("PENDING") &&
              x.toUpperCase().includes("PENDING")))
        );
      ...
      if (threadProtocolAlerts) {
        threadProtocolAlerts.forEach((alert) => {
          msgObj.protocol[deployment]?.Field?.forEach((fieldName, idx) => {
            const alertStrAfterFieldName = alert
              ?.toUpperCase()
              ?.split(fieldName?.toUpperCase())[1];
            const alertStrBeforeDesc =
              alertStrAfterFieldName?.split("DESCRIPTION")[0];
            const alertToBeSent =
              msgObj?.protocol[deployment]?.value?.[idx]?.toUpperCase();
            const valueCondition = alertStrBeforeDesc?.includes(alertToBeSent);
            if (
              alert.toUpperCase().includes(fieldName.toUpperCase()) &&
              valueCondition
            ) {
        ...
        if (!invalidProtocolAlertIndexes[deployment].includes(idx)) {
          ...
          ghIssuePromiseArray.push(
            postGithubIssue(
              ...
              alertBody,
              postedIssues,
              deployment?.toUpperCase()?.includes("DECEN")
            )
          );
        }
      });
      if (validAlerts.length > 0) {
        let link = `https://subgraphs.messari.io/subgraph?endpoint=messari/${
          msgObj.protocolName
        }-${deployment}&tab=protocol${
          deployment.toUpperCase().includes("PENDING") ? "&version=pending" : ""
        }`;
        messageConstruction += `\n${deployment}: ${link}\n${validAlerts.join(
          "\n"
        )}`;
    ...
    if (messageConstruction.length > 0) {
      messageConstruction =
        `Protocol Errors on ${msgObj.protocolName}\n` + messageConstruction;
    }
  }
    ...
    const req = await axios
      ...
      .catch(async function (err) {
        ...
        await axios
          .post(baseURL, postJSON, { headers: { ...headers } })
          .catch((zapierErr) =>
            console.log("ERROR ZAPIER MSG # " + zapierErr?.message)
          );
      });
    return req;
  } catch (err) {
