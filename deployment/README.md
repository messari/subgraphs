...
- _Protocols/`protocol`/config/networks/`network`/`network`.json_
  ...
  - If using a configurations/ folder for protocol and network specific constants, add a constant of the form to the `network`.json file:
    - "deployment": "{PROTOCOL*SLUG}*{NETWORK}"
- _Protocols/`protocol`/config/networks/`network`/`network`.ts_
...
- _Protocols/`protocol`/config/templates/`protocol`.template.yaml_
...
- _configurations/_
  ...
  - _configurations/_
    ...
    - _deploy.ts_
      ...
      - Contains a namespace for {PROTOCOL*SLUG}*{NETWORK} used in configure.template.ts
...
- _HEAD:/deployment/deploymentConfigurations.json_
