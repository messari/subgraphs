...
import { Account, AccountBalance, AccountBalanceDailySnapshot, Token } from '../../generated/schema'
...
export function updateAccountBalanceDailySnapshot(balance: AccountBalance, event: ethereum.Event): void {
...
export function getOrCreateAccountBalanceDailySnapshot(balance: AccountBalance, block: ethereum.Block): AccountBalanceDailySnapshot {
  let snapshotId = balance.account + '-' + balance.token + '-' + (block.timestamp.toI64() / SECONDS_PER_DAY).toString();
