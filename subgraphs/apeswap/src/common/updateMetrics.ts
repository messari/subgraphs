...
import { Account, DailyActiveAccount, LiquidityPool, UsageMetricsDailySnapshot, _HelperStore, _TokenTracker } from "../../generated/schema"
import { getLiquidityPool, getLiquidityPoolAmounts, getOrCreateDex, getOrCreateEtherHelper, getOrCreateFinancials, getOrCreatePoolDailySnapshot, getOrCreateRewardToken, getOrCreateToken, getOrCreateTokenTracker, getOrCreateUsersHelper } from "./getters"
import { BIGDECIMAL_ZERO, BIGINT_ZERO, DEFAULT_DECIMALS, INT_ONE, SECONDS_PER_DAY } from "./constants"
...
export function updateTvlAndTokenPrices(poolAddress: Address, blockNumber: BigInt): void {
  ...
  // Uncomment to include use of Pheonix's price oracle
  // if (blockNumber.toI32() > 10207858) {
  //   let fetchPrice0 = getUsdPricePerToken(Address.fromBytes(token0.id));
