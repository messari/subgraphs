...
export function getRewardsPerDay(
  ...
  let unnormalizedBlockSpeed = WINDOW_SIZE_SECONDS_BD.div(windowSecondsCount).times(
    windowBlocksCount,
  );

  // block speed converted to specified rate.
  let normalizedBlockSpeed = RATE_IN_SECONDS_BD.div(WINDOW_SIZE_SECONDS_BD).times(
