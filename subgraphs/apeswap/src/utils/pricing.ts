...
export function getBnbPriceInUSD(): BigInt {
  ...
  let usdtPair = PancakeswapPair.load(
    Address.fromString(USDT_WBNB_PAIR).toHexString()
  ); // usdt is token0
  let busdPair = PancakeswapPair.load(
  ...
  if (busdPair !== null && usdtPair !== null) {
    ...
    if (totalLiquidityBNB.notEqual(BIGINT_ZERO)) {
...
export function findBnbPerToken(token: Token): BigInt {
  ...
  for (let i = 0; i < WHITELIST.length; ++i) {
    ...
    if (pairAddress.toHex() != ZERO_ADDRESS) {
      ...
      if (pool !== null) {
        if (pool._token0 == token.id && pool._reserveBNB.gt(MINIMUM_LIQUIDITY_THRESHOLD_BNB)) {
        ...
        if (pool._token1 == token.id && pool._reserveBNB.gt(MINIMUM_LIQUIDITY_THRESHOLD_BNB)) {
...
export function getTrackedVolumeUSD(
  ...
  if (pool.liquidityProviderCount.lt(BigInt.fromI32(5))) {
    ...
    if (WHITELIST.includes(token0.id) && !WHITELIST.includes(token1.id)) {
      if (reserve0USD.times(BigInt.fromI32(2)).lt(MINIMUM_USD_THRESHOLD_NEW_PAIRS)) {
        return BIGINT_ZERO
      }
    }
    if (!WHITELIST.includes(token0.id) && WHITELIST.includes(token1.id)) {
      if (reserve1USD.times(BigInt.fromI32(2)).lt(MINIMUM_USD_THRESHOLD_NEW_PAIRS)) {
        return BIGINT_ZERO
      }
    }
  }
