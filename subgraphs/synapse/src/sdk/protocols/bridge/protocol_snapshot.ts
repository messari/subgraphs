...
const SECONDS_IN_HOUR = BigInt.fromI32(3600);
...
export class ProtocolSnapshot {
  ...
  private takeFinancialsDailySnapshot(): void {
    ...
    const day = event.block.timestamp.div(SECONDS_IN_DAY).toI32();
  ...
  private takeUsageHourlySnapshot(): void {
    ...
    const event = this.event;
    const protocol = this.protocol;
    const hour = event.block.timestamp.div(SECONDS_IN_HOUR).toI32();
