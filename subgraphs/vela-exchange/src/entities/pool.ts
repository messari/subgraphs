...
export function getOrCreateLiquidityPool(event: ethereum.Event): LiquidityPool {
  ...
  const poolAddress = Bytes.fromHexString(VAULT_ADDRESS);
...
export function addPoolInputToken(
  event: ethereum.Event,
  pool: LiquidityPool,
  inputToken: Token
): void {
  ...
  if (inputTokenIndex >= 0) {
    log.error("Input token exist for {}", [inputToken.name]);
    return;
  }
  ...
  inputTokenWeights.push(BIGDECIMAL_HUNDRED);
  ...
  log.error("Input token does not exist for {}", [inputToken.name]);
  pool._lastUpdateTimestamp = event.block.timestamp;
  pool.save();
}
...
export function updatePoolTvl(
  ...
  const tryTotalUSDC = vaultContract.try_totalUSDC();
  if (!tryTotalUSDC.reverted) {
    const inputTokenBalances = pool.inputTokenBalances;
    inputTokenBalances[0] = tryTotalUSDC.value;
    pool.inputTokenBalances = inputTokenBalances;
  ...
  const tryTotalVLP = vaultContract.try_totalVLP();
  if (!tryTotalVLP.reverted) {
    pool.outputTokenSupply = tryTotalVLP.value;
    pool.outputTokenPriceUSD = pool.totalValueLockedUSD.div(
      convertTokenToDecimal(pool.outputTokenSupply!, outputToken.decimals)
    );
  }
