...
import { UniswapPair as UniswapPairContract } from "../../../generated/HopL1Bridge/UniswapPair";
import { UniswapRouter as UniswapRouterContract } from "../../../generated/HopL1Bridge/UniswapRouter";
...
export function getTokenPriceUSDC(tokenAddress: Address): CustomPriceType {
  ...
  if (isLpToken(tokenAddress, config.wethAddress())) {
    return getLpTokenPriceUsdc(tokenAddress);
  }
...
export function getPriceFromRouter(
  ...
  if (token0Address == ethAddress) token0Address = ethAddress;
  if (token1Address == ethAddress) token1Address = ethAddress;
