...
export class Pool {
	...
	getInputToken(): Token {
		return this.tokens.getOrCreateToken(Address.fromBytes(this.pool.inputToken))
	}
	...
	private _addInputTokenLiquidityBalance(delta: BigInt): void {
		this.pool._inputTokenLiquidityBalance = this.pool._inputTokenLiquidityBalance!.plus(
			delta
		)
		this.save()
	}
	...
	addVolume(
		...
		if (isOutgoing) {
			...
			route.cumulativeVolumeOutUSD = route.cumulativeVolumeOutUSD.plus(
				amountUSD
			)
			this.pool.cumulativeVolumeOut = this.pool.cumulativeVolumeOut.plus(amount)
			this.pool.cumulativeVolumeOutUSD = this.pool.cumulativeVolumeOutUSD.plus(
				amountUSD
			)
			...
			this.pool.cumulativeVolumeInUSD = this.pool.cumulativeVolumeInUSD.plus(
				amountUSD
			)
	...
	addSupplySideRevenueUSD(rev: BigDecimal): void {
		this.pool.cumulativeTotalRevenueUSD = this.pool.cumulativeTotalRevenueUSD.plus(
			rev
		)
	...
	addStakedOutputTokenAmount(amount: BigInt): void {
		...
		this.pool.stakedOutputTokenAmount = this.pool.stakedOutputTokenAmount!.plus(
			amount
		)
		this.save()
	}
