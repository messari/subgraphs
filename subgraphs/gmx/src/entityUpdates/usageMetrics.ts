...
export function updateProtocolAndPoolTvl(
  ...
  pool.totalValueLockedUSD = bigIntToBigDecimal(aumInUsdg, DEFAULT_DECIMALS);
  ...
  if (outputTokenSupply != BIGINT_ZERO) {
    ...
    pool.outputTokenPriceUSD = pool.totalValueLockedUSD.div(
      bigIntToBigDecimal(outputTokenSupply, outputToken.decimals)
    );

    outputToken.lastPriceUSD = pool.outputTokenPriceUSD;
    outputToken.lastPriceBlockNumber = block.number;
    outputToken.save();
  }
  pool.save();
