...
export function calculateOutputTokenPriceInUSD(
  contractAddress: Address
): BigDecimal {
  ...
  if (
    yakStrategyV2Contract.try_depositToken().reverted ||
    yakStrategyV2Contract.try_getSharesForDepositTokens(DEFUALT_AMOUNT).reverted
  ) {
    return ZERO_BIGDECIMAL;
  } else {
    const depositTokenPrice: BigDecimal = calculatePriceInUSD(
      yakStrategyV2Contract.depositToken(),
      DEFUALT_AMOUNT
    );
    const getSharesForDepositToken: BigInt =
      yakStrategyV2Contract.getSharesForDepositTokens(DEFUALT_AMOUNT);
    const getSharesForDepositTokenInDecimal: BigDecimal =
      convertBigIntToBigDecimal(getSharesForDepositToken, 18);

    if (getSharesForDepositTokenInDecimal != ZERO_BIGDECIMAL) {
      outputTokenPriceInUSD = depositTokenPrice.div(
        getSharesForDepositTokenInDecimal
      );
    } else {
