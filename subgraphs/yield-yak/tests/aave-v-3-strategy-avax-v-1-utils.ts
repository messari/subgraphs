...
export function createAddRewardEvent(
  ...
  addRewardEvent.parameters.push(
    new ethereum.EventParam("swapPair", ethereum.Value.fromAddress(swapPair))
  )
...
export function createRemoveRewardEvent(rewardToken: Address): RemoveReward {
  let removeRewardEvent = changetype<RemoveReward>(newMockEvent())

  removeRewardEvent.parameters = new Array()

  removeRewardEvent.parameters.push(
    new ethereum.EventParam(
      "rewardToken",
      ethereum.Value.fromAddress(rewardToken)
    )
