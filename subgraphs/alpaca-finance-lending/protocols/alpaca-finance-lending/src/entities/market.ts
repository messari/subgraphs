...
export function createMarket(
    ...
    const id = poolAddress.toHexString();
    let market = Market.load(id);

    if (!market) {
        const token = getOrCreateToken(inputToken);
        ...
        market.outputToken = getOrCreateToken(poolAddress, inputToken.toHexString()).id;
        market.rates = [];
...
export function updateInputTokenBalance(
    market: Market,
    vaultInstance: VaultContract
): Market {
    const totalToken = vaultInstance.try_totalToken();
    if (totalToken.reverted) {
        return market;
    }
    market.inputTokenBalance = totalToken.value;
    return market;
}
