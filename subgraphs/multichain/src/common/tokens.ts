...
export function fetchTokenSymbol(tokenAddress: Address): string {
  ...
  if (!symbolResultBytes.reverted) {
    ...
    if (!isNullEthValue(symbolResultBytes.value.toHexString())) {
      ...
      log.warning("[fetchTokenSymbol] checking static definition for {}", [
...
export function fetchTokenName(tokenAddress: Address): string {
  ...
  if (!nameResultBytes.reverted) {
    ...
    if (!isNullEthValue(nameResultBytes.value.toHexString())) {
      ...
      // try with the static definition
      log.warning("[fetchTokenName] checking static definition for {}", [
        tokenAddress.toHexString(),
      ]);
      let staticTokenDefinition =
        StaticTokenDefinition.fromAddress(tokenAddress);
...
export function fetchTokenDecimals(tokenAddress: Address): i32 {
  ...
  // try with the static definition
  log.warning("[fetchTokenDecimals] checking static definition for {}", [
    tokenAddress.toHexString(),
  ]);
