...
import { morphoPositionsFromProtocol } from "../protocols/morpho/src/common";
import { getMarket, getOrInitToken } from "./utils/initializers";
import { PositionSide } from "./sdk/protocols/lending/constants";
...
function snapshotPosition(position: Position, event: ethereum.Event): void {
  ...
  const balanceOnPool = position._balanceOnPool!.times(poolIndex!).div(RAY_BI);
  const balanceInP2P = position._balanceInP2P!.times(p2pIndex!).div(RAY_BI);
