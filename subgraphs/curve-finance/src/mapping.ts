...
export function addRegistryPool(pool: Address, registry: Address, block: BigInt, timestamp: BigInt, hash: Bytes): void {
  ...
  if (isLendingPool(pool)) {
    // Lending pool
    log.info("New lending pool {} added from registry at {}", [pool.toHexString(), hash.toHexString()]);
  ...
  if (!testMetaPoolResult.reverted || unknownMetapool) {
    ...
    ensureBasePoolTracking(basePool, registry, timestamp, block, hash, basePoolGauge);
    createNewRegistryPool(
      pool,
      basePool,
...
export function handleMainRegistryPoolAdded(event: PoolAdded): void {
  addRegistryPool(event.params.pool, event.address, event.block.number, event.block.timestamp, event.transaction.hash);
}
