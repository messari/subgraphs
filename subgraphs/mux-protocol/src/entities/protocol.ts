...
export function getOrCreateProtocol(): DerivPerpProtocol {
  const poolAddress = NetworkConfigs.getPoolAddress();
  let protocol = DerivPerpProtocol.load(poolAddress);

  if (!protocol) {
    protocol = new DerivPerpProtocol(poolAddress);
...
export function updateProtocolOpenInterestUSD(
  ...
  protocol.totalOpenInterestUSD = protocol.longOpenInterestUSD.plus(
    protocol.shortOpenInterestUSD
  );
