...
import {
  ...
  WithdrawToL1,
  WithdrawToL2,
} from "../../generated/FusionLock/FusionLock";
import { _ERC20 } from "../../generated/FusionLock/_ERC20";
...
export function handleDeposit(event: Deposit): void {
  ...
  const user = event.params.depositOwner;
  const account = sdk.Accounts.loadAccount(user);
  account.trackActivity();
}

export function handleWithdrawToL1(event: WithdrawToL1): void {
  const sdk = SDK.initializeFromEvent(
    conf,
    new Pricer(),
    new TokenInit(),
  ...
  const user = event.params.owner;
  const account = sdk.Accounts.loadAccount(user);
  account.trackActivity();
}

export function handleWithdrawToL2(event: WithdrawToL2): void {
  const sdk = SDK.initializeFromEvent(
    conf,
    new Pricer(),
    new TokenInit(),
  ...
  let tokenAddr = event.params.l1Token;
  if (tokenAddr == Address.fromString(ZERO_ADDRESS)) {
    tokenAddr = Address.fromString(ETH_ADDRESS);
  }
  const token = sdk.Tokens.getOrCreateToken(tokenAddr);
