...
export function handleRewardsClaimed(event: RewardsClaimed): void {
  ...
  const tokenInstance = initToken(Address.fromString(rewardToken.id));
...
export function handleRewardsAccrued(event: RewardsAccrued): void {
  ...
  const token = initToken(Address.fromString(rewardToken.id));
  // !!! These values need to be added to market, snapshots etc
  ...
  const financialsDailySnapshot = updateFinancials(event);
