// CURRENT METHOD OF FETCHING REWARD EMISSIONS MAKES THIS MAPPING USELESS, COMMENTING OUT AND KEEPING FOR NOW
...
//   const rewardTokens = getRewardTokensFromIncController(market);
//   if (!rewardTokens) {
...
//   const rewardAccruedInUSD: BigDecimal[] = [amountInUSD(token.id, token.decimals, event.params.amount), amountInUSD(token.id, token.decimals, event.params.amount)];
