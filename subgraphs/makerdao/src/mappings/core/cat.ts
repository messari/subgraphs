...
import { BIGDECIMAL_ONE_HUNDRED, BIGDECIMAL_ONE, MCD_CAT_ADDRESS, MCD_CAT_V2_ADDRESS } from "../../common/constants";
...
export function handleFileDog(event: File2): void {
  ...
  if (what == 'chop') {
      let market = getMarketFromIlk(ilk)
      market.liquidationPenalty = (bigIntToBigDecimal(event.params.data,DEFAULT_DECIMALS).minus(BIGDECIMAL_ONE)).times(BIGDECIMAL_ONE_HUNDRED)
      market.save()
  }
}
