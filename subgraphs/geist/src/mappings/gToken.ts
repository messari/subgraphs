import {
  ...
  export function handleGTokenMint(event: Mint): void {
    ...
    log.info(event.params.from.toHexString() + ' ' + event.transaction.from.toHexString() + ' MINTING gTOKEN: ' + event.address.toHexString(), []);
  ...
  export function handleGTokenBurn(event: Burn): void {
    ...
    log.info(event.params.from.toHexString() + ' ' + event.transaction.from.toHexString() + ' BURNING gTOKEN: ' + event.address.toHexString(), []);
    updateOutputTokenSupply(event);
  }