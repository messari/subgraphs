import {
  ...
  export function getCurrentRewardEmissions(market: Market): BigInt[] {
    ...
    // From the incentives controller contract, get pull the 'assets' values from the market GToken, sToken, and vToken
    const incentivesControllerContract = IncentivesControllerContract.bind(incentivesController);
    if (!incentivesControllerContract.try_rewardTokens(Address.fromString(market.outputToken)).reverted) {
  ...
  export function updateOutputTokenSupply(event: ethereum.Event): void {
    ...
    const gTokenInstance = GToken.bind(outputTokenAddr);
    const tryTokenSupply = gTokenInstance.try_totalSupply();
    if (!tryTokenSupply.reverted) {
      const gToken = getOrCreateToken(outputTokenAddr);
  ...
  export function initIncentivesController(market: Market): Address {
    // This function attempts to pull the incentives controller from the gToken/output token
    log.info('INIT INCENTIVE CONT FROM MARKET: ' + market.id + ' gToken: ' + market.outputToken, []);
    const gToken = GToken.bind(Address.fromString(market.outputToken));
    if (!gToken.try_getIncentivesController().reverted) {
      const incContAddr = gToken.try_getIncentivesController().value;
      log.info('INCENTIVE CONTROLLER SUCCESS ' + incContAddr.toHexString(), []);
      return incContAddr;
    } else {
      log.info('FAILED TO GET INCENTIVE CONTROLLER ' + gToken._address.toHexString() + ' ' + market.id, []);
  ...
  export function updateMetricsDailySnapshot(event: ethereum.Event): UsageMetricsDailySnapshot {
    ...
    let accountActive = ActiveAccount.load(daysSinceEpoch + '-' + userAddr.toHexString());
    if (!accountActive) {
      accountActive = new ActiveAccount(daysSinceEpoch + '-' + userAddr.toHexString());
      accountActive.save();
  ...
  export function getAssetPriceInUSDC(tokenAddress: Address): BigDecimal {
    ...
    const tryPriceUSDCInEth = oracle.try_getAssetPrice(Address.fromString(TOKEN_ADDRESS_USDC));
  ...
  export function amountInUSD(priceInUSDC: BigDecimal, decimals: number, amount: BigInt, market: Market): BigDecimal {
    ...
    log.info(market.id + ' TOKEN AMOUNT ' + amount.toString() + ' TIMES PRICE IN USD ' + priceInUSDC.toString() + ' === ' + amountUSD.toString() + ' ' + market.inputTokens.length.toString(), []);
  ...
  export function getCurrentRewardEmissionsUSD(market: Market): BigDecimal[] {
    ...
    rewardEmissionsUSD[0] = amountInUSD(rewardPriceInUSDC, rewardToken.decimals, market.rewardTokenEmissionsAmount[0], market);
    rewardEmissionsUSD[1] = amountInUSD(rewardPriceInUSDC, rewardToken.decimals, market.rewardTokenEmissionsAmount[1], market);
  ...
  export function calculateRevenues(market: Market, token: Token): void {
    ...
    const dataProviderContract = AaveProtocolDataProvider.bind(Address.fromString(AAVE_PROTOCOL_DATA_PROVIDER));

    const reserve_data = dataProviderContract.try_getReserveData(Address.fromString(market.id));
    ...
    const varAmountUSD = amountInUSD(market.inputTokenPricesUSD[0], token.decimals, market.totalVariableValueLocked, market);
    ...
    const staAmountUSD = amountInUSD(market.inputTokenPricesUSD[0], token.decimals, market.totalStableValueLocked, market);
