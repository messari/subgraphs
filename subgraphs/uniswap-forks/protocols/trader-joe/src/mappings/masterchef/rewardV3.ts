...
import { Deposit, Withdraw, EmergencyWithdraw, Add, Set } from "../../../../../generated/MasterChefV3/MasterChefV3TraderJoe";
...
export function handleAdd(event: Add): void {
  let masterChefV3Pool = createMasterChefStakingPool(event, MasterChef.MASTERCHEFV3, event.params.pid, event.params.lpToken);
  updateMasterChefTotalAllocation(event, masterChefV3Pool.poolAllocPoint, event.params.allocPoint, MasterChef.MASTERCHEFV3);
  masterChefV3Pool.poolAllocPoint = event.params.allocPoint;
  masterChefV3Pool.save();
}

export function handleSet(event: Set): void {
  let masterChefV3Pool = _MasterChefStakingPool.load(MasterChef.MASTERCHEFV3 + "-" + event.params.pid.toString())!
  updateMasterChefTotalAllocation(event, masterChefV3Pool.poolAllocPoint, event.params.allocPoint, MasterChef.MASTERCHEFV3);
  masterChefV3Pool.poolAllocPoint = event.params.allocPoint;
  masterChefV3Pool.save();
}
