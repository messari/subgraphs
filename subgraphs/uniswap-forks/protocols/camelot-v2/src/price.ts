    ...
    for (let i = 0; i < whiteList.length; ++i) {
      ...
      if (pool.outputTokenSupply!.gt(BIGINT_ZERO)) {
        ...
        if (
          ...
          const pair = Pair.bind(Address.fromString(pool.id));
          const stable = pair.stableSwap();

          let newPrice = BIGDECIMAL_ZERO;
          if (stable) {
            newPrice = calculateRateForStablePair(
              pool,
              pair,
              token,
              whitelistToken
...
function calculateRateForStablePair(
  ...
  amount = amount
    ...
    .truncate(0).digits;
  const rate = pair.getAmountOut(amount, Address.fromString(tokenIn.id));
