import { Protocol } from "../../src/common/constants";
import { ApeswapConfigurations } from "../apeswap/apeswapConfigurations";
import { SushiswapConfigurations } from "../sushiswap/sushiswapConfigurations";
import { UniswapConfigurations } from "../uniswap-v2/uniswapConfigurations";
import { ProtocolNetworkFieldMap } from "./types";

export const AllConfigurations: ProtocolNetworkFieldMap = {
    [Protocol.APESWAP]: ApeswapConfigurations,
    [Protocol.SUSHISWAP]: SushiswapConfigurations,
    [Protocol.UNISWAP_V2]: UniswapConfigurations,
  }