...
export class Pool {
  ...
  initialize(
    ...
    this.pool._lastRewardUpdateTimestamp = BIGINT_ZERO;
    this.save();
  ...
  getTotalValueLocked(): BigDecimal {
    return this.pool.totalValueLockedUSD;
  }
  ...
  getInputTokenBalances(): BigInt[] {
    return this.pool.inputTokenBalances;
  }
  ...
  addInputTokenBalances(
    ...
    for (let idx = 0; idx < this.pool.inputTokenBalances.length; idx++) {
      newBalances[idx] = this.pool.inputTokenBalances[idx].plus(amounts[idx]);
    }
  ...
  getProtocolSideRevenueUSD(): BigDecimal {
    return this.pool.cumulativeProtocolSideRevenueUSD;
  }
   ...
   */
  refreshOutputTokenPriceUSD(token: Token): void {
  ...
  getlastRewardUpdateTimestamp(): BigInt {
    return this.pool._lastRewardUpdateTimestamp;
  }

  setlastRewardUpdateTimestamp(timestamp: BigInt): void {
    this.pool._lastRewardUpdateTimestamp = timestamp;
    this.save();
  }
}
