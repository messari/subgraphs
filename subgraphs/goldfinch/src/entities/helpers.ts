...
import { VERSION_BEFORE_V2_2 } from "../common/utils";
...
export function getTotalDeposited(
  ...
  for (let i = 0, k = juniorTranches.length; i < k; ++i) {
    ...
    if (!jrTranche || !srTranche) {
      throw new Error(
        `Missing tranche information for ${address.toHexString()}`
      );
    }
...
export function getJuniorDeposited(
  juniorTranches: JuniorTrancheInfo[]
): BigInt {
  let juniorDeposited = BigInt.zero();
  for (let i = 0; i < juniorTranches.length; i++) {
    juniorDeposited = juniorDeposited.plus(
      juniorTranches[i].principalDeposited
    );
  }
...
export function calculateEstimatedInterestForTranchedPool(
