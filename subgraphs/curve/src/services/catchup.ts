...
export function catchUpRegistryMainnet(): void {
  log.warning('Catching up registry...',[])
  for (let i = 0; i < CATCHUP_POOLS.length; i++) {
    log.warning('Manually adding pool {}', [CATCHUP_POOLS[i].toHexString()])
    ...
    log.warning('LP Token: {} for {}', [lpToken.toHexString(),CATCHUP_POOLS[i].toHexString()])
    const lpTokenContract = ERC20.bind(lpToken)
    CurvePoolTemplate.create(CATCHUP_POOLS[i])
    log.warning('createNewPool {}', [CATCHUP_POOLS[i].toHexString()])
    createNewPool(
      CATCHUP_POOLS[i],
      lpToken,
