...
import { BIGDECIMAL_ZERO, BIGINT_ZERO, CRV_POOL_START_BLOCK, CRV_TOKEN, DEFAULT_DECIMALS, SECONDS_PER_DAY, ZERO_ADDRESS } from '../../common/constants'
...
export function getCrvInflationRateUSD(crvTokenMinted:BigInt, blockNumber:BigInt,timestamp: BigInt): BigDecimal {
    const crvPrice = blockNumber.toI32() > CRV_POOL_START_BLOCK ? getTokenPriceSnapshot(CRV_TOKEN, timestamp, false) : BIGDECIMAL_ZERO
...
export function setStakedOutputTokenAmount(pool: LiquidityPool): void {
    ...
    const stakedOutputTokenAmount = stakedOutputTokenAmountCall.reverted ? BIGINT_ZERO : stakedOutputTokenAmountCall.value;
