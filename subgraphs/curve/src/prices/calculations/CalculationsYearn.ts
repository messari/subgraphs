...
export function getTokenPriceFromCalculationYearn(
    tokenAddr: Address,
    network: string
  ): CustomPriceType {
    ...
    let underlyingToken: Address = utils.readValue<Address>(calculationYearnContract.try_underlying(),constants.ZERO_ADDRESS)
    if (underlyingToken == constants.ZERO_ADDRESS) {
      return new CustomPriceType()
    }
    let exchangeRateStored: BigDecimal = utils.readValue<BigInt>(
        calculationYearnContract.try_exchangeRateStored(),
    ...
    .div(
      constants.BIGINT_TEN.pow(
        decimalsAdjustment.plus(exchangeRateMantissa).toI32() as u8
      ).toBigDecimal()
    );
    return CustomPriceType.initialize(tokenPrice);
  }

export function getTokenPriceFromCalculationYearnV1(
    tokenAddr: Address,
    network: string
  ): CustomPriceType {
    let tokensMapping = constants.WHITELIST_TOKENS_MAP.get(network);
    ...
    let underlyingToken: Address = utils.readValue<Address>(calculationYearnContract.try_aaveToken(),constants.ZERO_ADDRESS)
    if (underlyingToken == constants.ZERO_ADDRESS) {
      return new CustomPriceType()
    }
    let pricePerFullShare: BigDecimal = utils.readValue<BigInt>(
        calculationYearnContract.try_getPricePerFullShare(),
      constants.BIGINT_ZERO
    ).toBigDecimal();
