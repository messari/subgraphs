...
export function getEthRateUniV3(token: Address): BigDecimal {
  ...
  const rate = quoter.try_quoteExactInputSingle(token, WETH_ADDRESS, fee, exponentToBigInt(decimals.toI32()), BIG_INT_ZERO)
  if (!rate.reverted) {
...
export function getTokenAValueInTokenB(tokenA: Address, tokenB: Address): BigDecimal {
  ...
  return ethRateA.div(ethRateB).times(exponentToBigDecimal(decimalsA.toI32())).div(exponentToBigDecimal(decimalsB.toI32()))
}
