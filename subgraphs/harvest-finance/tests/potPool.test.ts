...
import { handleStaked, handleWithdrawn } from '../src/potPool'
...
describe('PotPool', () => {
  ...
  describe('handleStaked', () => {
    test('Increment stakedOutputTokenAmount', () => {
      ...
      helpers.mocking.potPool.potPool(
        poolAddress,
        tokenAddress,
      ...
      const event = helpers.mocking.potPool.createStakedEvent(
        poolAddress,
        Address.zero(),
        amount
      )
  ...
  describe('handleWithdrawn', () => {
    test('Decrement stakedOutputTokenAmount', () => {
      ...
      //Create event
      const amount = BigInt.fromI32(100)
      const event = helpers.mocking.potPool.createWithdrawnEvent(
        poolAddress,
        Address.zero(),
        amount
      )
