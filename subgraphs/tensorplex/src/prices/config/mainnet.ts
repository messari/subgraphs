...
class TaoOverride implements OracleConfig {
  oracleCount(): number {
    return constants.INT_ONE;
  }
  oracleOrder(): string[] {
    return [constants.OracleType.YEARN_LENS_ORACLE];
  }
}
  ...
  getOracleOverride(
    ...
    if (tokenAddr || block) {
      ...
      if (
        tokenAddr &&
        [
          Address.fromString("0x77e06c9eccf2e797fd462a92b6d7642ef85b0a44"), // wTAO
        ].includes(tokenAddr)
      ) {
        return new TaoOverride();
      }
    }
