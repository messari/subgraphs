...
import { EigenConfig } from "../../generated/EigenConfig/EigenConfig";
...
export function getReceiptByAsset(asset: Address): Address {
  const contract = EigenConfig.bind(
    Address.fromString(constants.EIGEN_CONFIG_ADDRESS)
  );

  const receipt = readValue<Address>(
    contract.try_mLRTReceiptByAsset(asset),
