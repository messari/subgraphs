...
export class Pool {
   ...
   */
  addInputToken(inputToken: Token, amount: BigInt): void {
    if (this.pool.inputTokens.includes(inputToken.id)) {
      const index = this.pool.inputTokens.indexOf(inputToken.id);
      this.pool.inputTokenBalances[index] =
        this.pool.inputTokenBalances[index].plus(amount);
    } else {
      this.pool.inputTokens = this.pool.inputTokens.concat([inputToken.id]);
      this.pool.inputTokenBalances = this.pool.inputTokenBalances.concat([
        amount,
      ]);
    }
