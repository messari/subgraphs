...
export function syncWithEulerGeneralView(
  ...
  log.warning("syncWithEulerGeneralView: eulerViewMarkets: {}", [eulerViewMarkets.toString()]);
  ...
  log.warning("syncWithEulerGeneralView: ethUsdcExchangeRate: {}", [ethUsdcExchangeRate.toString()]);
  ...
  for (let i = 0; i < eulerViewMarkets.length; i++) {
  ...
  log.warning("updateMarketUtility: {}", ["hi"]);
  protocol.save();
}
