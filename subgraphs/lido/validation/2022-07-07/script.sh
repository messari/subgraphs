...
{
    "query": "{
       poolHourlySnapshots (where: { timestamp_gt: $EPOCH_TODAY, timestamp_lt: $EPOCH_TOMORROW }) {
            id
            protocol { id }
            pool { id }
            blockNumber
...
printf "\n\n[+] SUBGRAPH: $2 ................."
curl -s -L -H "Content-Type: application/json" -X POST -d @data-full.json $2 | jq '.'


printf "\n\n[+] TOKEN TERMINAL............."
TIMESTAMP="$ZULU_TT_TODAY" jq '.[] | select(.timestamp == env.TIMESTAMP)' metrics.json | grep "timestamp\|revenue\|tvl"