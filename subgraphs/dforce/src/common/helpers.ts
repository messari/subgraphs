...
import { iToken, Mint, Redeem, Borrow, RepayBorrow, LiquidateBorrow } from "../../generated/templates/iToken/iToken";
import {
  Account,
  ActiveAccount,
  Market,
...
export function updateWithdraw(event: Redeem): void {
  ...
  if (withdraw == null) {
    ...
    withdraw.to = event.params.recipient.toHexString();
    ...
    withdraw.amount = event.params.redeemUnderlyingAmount;
...
export function updateLiquidate(event: LiquidateBorrow): void {
  ...
  let pricePerUnderlyingToken = getUnderlyingTokenPricePerAmount(event.address);
  let pricePerCollateralToken = getUnderlyingTokenPricePerAmount(event.params.iTokenCollateral);
  // get exchangeRate for collateral token
  let collateralMarketId = event.params.iTokenCollateral.toHexString();
  ...
  if (liquidate == null) {
    ...
    liquidate.asset = event.params.iTokenCollateral.toHexString();
...
export function updateUsageMetrics(event: ethereum.Event, user: Address): void {
  ...
  //usageDailyMetrics.dailyTransactionCount increment w/ specific event
  ...
  //usageHourlyMetrics.hourlyTransactionCount increment with specific event
  // Update the block number and timestamp to that of the last transaction of that day
  usageHourlyMetrics.blockNumber = event.block.number;
  usageHourlyMetrics.timestamp = event.block.timestamp;
  usageHourlyMetrics.save();
...
export function updateMarket(event: ethereum.Event, borrowAmount: BigInt = BIGINT_ZERO): void {
  let protocol = getOrCreateProtocol();
  // event must be emitted by a iToken/Market contract
  let marketId = event.address.toHexString();
  // alternatively, get marketId from dataSource.address
  let markets = protocol.markets;
  assert(markets.includes(event.address.toHexString()), "Event not emitted by a iToken contract");
  ...
  if (market != null) {
    ...
    // Have to use exchangeRateStored function here as exchangeRateCurrent function updates
    // interest accrued and needs to write to storage
    let tryExchangeRate = tokenContract.try_exchangeRateStored();
    ...
    // derive outputToken (iToken) price from exchange rate and inputTokenPriceUSD
    if (market.exchangeRate) {
      market.outputTokenPriceUSD = inputTokenPriceUSD.div(market.exchangeRate!);
    }

    //
    //Reward are updated in handleRewardDistributed() &
    // triggered by RewardDistributor.RewardDistributed
...
export function aggregateAllMarkets(event: ethereum.Event): void {
  ...
  let marketIds = protocol.markets;
