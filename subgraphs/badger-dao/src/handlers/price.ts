...
export function getPriceOfCurveLpToken(
  ...
  if (isBaseToken) {
    tokenPrice = readValue<BigInt>(
      curveContract.try_getBasePrice(tokenAddress),
      BigInt.zero(),
    );
  }
  ...
  if (isCToken) {
    tokenPrice = readValue<BigInt>(
      curveContract.try_getCurvePriceUsdc(tokenAddress),
      BigInt.zero(),
    );
  }
...
export function getVirtualPriceOfCurveLpToken(
  ...
  if (isCToken) {
    tokenPrice = readValue<BigInt>(
      curveContract.try_getVirtualPrice(tokenAddress),
      BigInt.zero(),
    );
  }

  log.debug('[BADGER] token virtual price for {} is {}', [
    tokenAddress.toHex(),
    tokenPrice.toString(),
  ]);
