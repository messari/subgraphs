        ...
        val.forEach((dep, idx) => {
          ...
          if (!formattedDeployments[Object.keys(liveDeployments)[idx]]) {
            formattedDeployments[Object.keys(liveDeployments)[idx]] = {
              IPFS_Hash: "N/A",
              issues: [
                "Could not pull either current subgraph IPFS hash given subgraph name. Query attempted on _meta entity at endpoint https://api.thegraph.com/subgraphs/name/" +
                  Object.keys(liveDeployments)[idx],
              ],
            };
          }
          if (listedNoName[Object.keys(liveDeployments)[idx]]) {
            formattedDeployments[Object.keys(liveDeployments)[idx]].issues.push(
              "deployment.json file does not have subgraph name string under the 'messari' key. Attempted to construct subgraph name from 'messari/(PROTOCOL)-(NETWORK)",
            );
          }
        });
