...
export const ChartContainer = ({
  ...
  const [csvMetaData, setCsvMetaData] = useState<{ fileName: string; columnName: string; csvError: string | null }>({
    fileName: "",
    columnName: "",
    csvError: null,
  });
  ...
  function jpegDownloadHandler(downloadInZip: boolean) {
    try {
      ...
      if (downloadInZip) {
        setChartsImageFiles((prevState: any) => ({
          ...prevState,
  ...
  useEffect(() => {
    try {
      ...
      if (
        ...
        compChartToSet = lineupChartDatapoints({
    ...
    if (dataChartCopy) {
      ...
      if (jsonToUse) {
        if (Array.isArray(jsonToUse)) {
          ...
          jsonToUse.forEach(
            (x: { [x: string]: any }) => (csvDataPointsByDate[moment.utc(x.date * 1000).format(formatStr)] = x.value),
          );
          ...
          csvColumnOptions = (
            <CsvOverlayColumnDropDown
              setSelectedColumn={(x: string) => setCsvMetaData({ ...csvMetaData, columnName: x })}
          ...
          if (csvMetaData.columnName) {
            ...
            if (jsonToUse) {
              jsonToUse?.date?.forEach((date: number, idx: number) => {
                if (!!jsonToUse) {
                  csvDataPointsByDate[moment.utc(date * 1000).format(formatStr)] =
      ...
      if (typeof dataChartCopy === "object" && !Array.isArray(dataChartCopy)) {
        ...
        if (jsonToUse) {
          if (Array.isArray(jsonToUse)) {
            ...
            jsonToUse.forEach(
              (x: { [z: string]: any }) =>
                (csvDataPointsByDate[moment.utc(x.date * 1000).format("YYYY-MM-DD")] = x.value),
            );
            ...
            if (csvMetaData.columnName) {
              ...
              if (jsonToUse) {
                jsonToUse.date.forEach(
                  (x: any, idx: number) =>
                    (csvDataPointsByDate[moment.utc(x * 1000).format("YYYY-MM-DD")] =
                      jsonToUse?.[csvMetaData.columnName]?.[idx]),
                );
              }
              csvArr = iterativeBaseData.map((point: any) => {
        ...
        <>
          <Box padding={2} sx={{ border: 1, maxWidth: "100%" }}>
            ...
            chartRef.current?.toBase64Image("image/jpeg", 1).toString() !==
              ...
              <StandardChart chartData={chartData} chartRef={chartRef} />
            )}
          </Box>
        </>
      );
  ...
  if (!csvColumnOptions && csvJSONProp && !csvJSON) {
    ...
    csvColumnOptions = (
      <CsvOverlayColumnDropDown
        setSelectedColumn={(columnName: string) => setCsvMetaData({ ...csvMetaData, columnName })}
        selectedColumn={csvMetaData.columnName}
        columnsList={columnsList}
      />
    );
            ...
            <div style={{ marginLeft: "35px" }}>
              <Tooltip placement="top" title={"Overlay chart with data points populated from a .csv file"}>
                <UploadFileCSV
                  ...
                  field={datasetLabel}
                  csvJSON={csvJSON}
                  setCsvJSON={(jsonArgument: { [x: string]: any } | null) => {
                    if (!jsonArgument) {
                      setCompChart({});
                      setCsvJSON(null);
                    } else {
              ...
              <Tooltip placement="top" title={"Chart can be dragged and dropped to another tab"}>
                <Button
              ...
              <Tooltip placement="top" title={"Show plot points on hover"}>
                <Button
