...
function ProtocolsListByTVL({ protocolsToQuery, getData }: ProtocolsListByTVLProps) {
  ...
  const defiLlamaTableRows: any[] = protocolsToDevelop.map((protocol) => {
    ...
    return (
      ...
      >
        <TableCell sx={{ padding: "0 0 0 6px", verticalAlign: "middle", height: "30px" }}>{protocol.name}</TableCell>
        <TableCell sx={{ padding: "0", paddingRight: "6px", textAlign: "right", display: "flex" }}>
          {protocol.chains.map((x: string) => {
            if (
              !Object.keys(NetworkLogos).includes(x.toLowerCase()) &&
              !Object.keys(networkMapping).includes(x.toLowerCase())
            ) {
              return null;
            }
            return (
        ...
        <TableCell sx={{ padding: "0", paddingRight: "6px", textAlign: "right" }}>${formatIntToFixed2(tvl)}</TableCell>
  ...
  const columnLabels: string[] = ["Name", "Chains", "TVL (Sum Across Networks)", "Schema Type"];
  ...
  if (defiLlamaTableRows.length === 0) {
    tableBody = (
      <Typography variant="h5" align="left" fontSize={22} sx={{ padding: "0 6px" }}>
    ...
    <div style={{ overflowX: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", margin: "40px 24px 10px 16px" }}>
        ...
        <ProtocolTypeDropDown
          ...
          setProtocolType={(x: string) => setProtocolType(x)}
      ...
      <TableContainer sx={{ my: 4, mx: 2 }} key={"TableContainer-DefiLlama"}>
        <div style={{ width: "97.5%" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Typography variant="h4" align="left" fontSize={26}>
              {currentProtocolType}
            </Typography>
            <Button
              variant="contained"
