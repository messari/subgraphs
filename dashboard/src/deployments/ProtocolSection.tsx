...
function ProtocolSection({
  ...
  if (showDeposDropDown) {
    const depoRowsOnProtocol = protocol.networks.map((depo: any) => {
      ...
      if (!!depo?.pendingIndexStatus) {
        ...
        if (
          (!depo?.versions?.schema || !latestSchemaVersions.includes(depo?.versions?.schema)) &&
        ...
        if (!isLoadedPending) {
          pendingRow = (
            ...
            >
              ...
              >
                {depo?.status === "prod" ? (
                  <img src="https://images.emojiterra.com/twitter/v13.1/512px/2705.png" height="24px" width="24px" />
                ) : (
                  <img
          ...
          curationElement = (
            <span style={{ display: "inline-flex", alignItems: "center", padding: "0px 10px", fontSize: "14px" }}>
              <Tooltip title="Current Curation Signal" placement="top">
                <span style={{ padding: "0", cursor: "default", fontWeight: "800" }}>{convertedSignalAmount}K GRT</span>
              </Tooltip>
            </span>
          );
        }
          ...
          <TableRow
            onClick={(event) => {
              if (event.ctrlKey) {
                if (!validationSupported) {
                  window.open(
                    "https://thegraph.com/hosted-service/subgraph/messari/" + depo.hostedServiceId + "?version=pending",
                    "_blank",
                  );
                  return;
                }
  ...
  if (protocol?.schemaVersions?.length > 0) {
    const schemaColored = protocol?.schemaVersions?.map((x: string, idx: number) => {
      ...
      return (
        <span key={subgraphName + "-protocol-schemaVerRow-" + idx} style={{ color: "#FFA500" }}>
          {x}
        </span>
      );
    });
    ...
    >
      ...
      <TableCell sx={{ padding: "0", paddingRight: "16px", textAlign: "right" }}>
        {protocol?.status ? (
          ...
          <img src="https://github.githubassets.com/images/icons/emoji/unicode/1f6e0.png" height="24px" width="24px" />
        )}
      </TableCell>
      <TableCell sx={{ padding: "0", paddingRight: "16px", textAlign: "right" }}></TableCell>
      <TableCell sx={{ padding: "0", paddingRight: "16px", textAlign: "right" }}></TableCell>
