...
export const IndexingErrorDisplay = ({ subgraphName }: IndexingErrorProps) => {
  ...
  const indexed = parseFloat(
    (
      (subgraphIndex.indexingStatusesForSubgraphName[0].chains[0].latestBlock.number /
        subgraphIndex.indexingStatusesForSubgraphName[0].chains[0].chainHeadBlock.number) *
      100
    ).toFixed(2),
  );
  return (
    <>
      ...
      >
        <h3 style={{ textAlign: "center", color: "red" }}>{subgraphName}</h3>
          ...
          {subgraphIndex.indexingStatusesForSubgraphName[0].fatalError ? (
            <>
              <span style={{ color: "red" }}>
                ...
                {subgraphIndex.indexingStatusesForSubgraphName[0].fatalError.block.number}
              </span>{" "}
              -{" "}
              <span>
        ...
        <p>
          Network: {subgraphIndex.indexingStatusesForSubgraphName[0].chains[0].network} - Current chain block:{" "}
          {subgraphIndex.indexingStatusesForSubgraphName[0].chains[0].chainHeadBlock.number}
        </p>
