...
const messagesByLevel = (issuesArray: { message: string, type: string, level: string, fieldName: string }[]): JSX.Element[] => {
  const issuesMsgs = [];
...
export const IssuesDisplay = ({ issuesArray }: IssuesProps) => {
  ...
  if (criticalMsgs.length > 0) {
    criticalElement = <div style={{ borderBottom: 'black 2px solid' }}><h3>Critical:</h3><ol>{criticalMsgs}</ol></div>
  }

  let errorElement = null;
  if (errorMsgs.length > 0) {
    errorElement = <div style={{ borderBottom: 'black 2px solid' }}><h3>Error:</h3><ol>{errorMsgs}</ol></div>
  }

  let warningElement = null;
  if (warningMsgs.length > 0) {
    warningElement = <div style={{ borderBottom: 'black 2px solid' }}><h3>Warning:</h3><ol>{warningMsgs}</ol></div>
  }
  ...
  if (issuesDisplayCount > 0) {
    return (
      ...
      >
        <h3>
          DISPLAYING {issuesDisplayCount} Issues.
        </h3>
        {criticalElement}
        {errorElement}
        {warningElement}
